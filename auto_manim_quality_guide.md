# auto_manim 动画质量提升指南 v2.0

> 面向 ManimGL + AutoScene 的**高质量动画制作**全方位指南，涵盖工作流优化、视觉设计、动态策略、时间轴控制、性能调优等各个维度。

---

## 📋 目录

- [0. 质量目标与输出标准](#0-质量目标与输出标准)
- [1. 工作流分层与质量门槛](#1-工作流分层与质量门槛)
- [2. 输入素材处理优化](#2-输入素材处理优化)
- [3. 文案与时间轴优化](#3-文案与时间轴优化)
- [4. 画面排版与视觉层级](#4-画面排版与视觉层级)
- [5. 动画衔接与节奏控制](#5-动画衔接与节奏控制)
- [6. 可视化区动态策略](#6-可视化区动态策略)
- [7. 字幕与配音系统](#7-字幕与配音系统)
- [8. 相机与构图控制](#8-相机与构图控制)
- [9. 音效与配乐策略](#9-音效与配乐策略)
- [10. 视觉引导与标注系统](#10-视觉引导与标注系统)
- [11. 辉光与特效使用](#11-辉光与特效使用)
- [12. 颜色与配色策略](#12-颜色与配色策略)
- [13. 代码组织与可维护性](#13-代码组织与可维护性)
- [14. 性能与渲染稳定性](#14-性能与渲染稳定性)
- [15. 自动化工具与检查器](#15-自动化工具与检查器)
- [16. 交付前检查清单](#16-交付前检查清单)
- [17. 常见问题与解决方案](#17-常见问题与解决方案)
- [18. 后续可落地优化项](#18-后续可落地优化项)

---

## 0. 质量目标与输出标准

### 0.1 核心质量指标

| 维度 | 目标 | 量化标准 |
|------|------|----------|
| **信息清晰度** | 题面、条件、推导与结论一眼可读 | 单屏信息密度 ≤ 6 个视觉元素 |
| **视觉层级** | 标题 > 题目 > 可视化 > 推导 > 字幕 | 字号递减比例 1.2:1 |
| **时序对齐** | 音画完美同步 | 误差 ≤ 0.2s |
| **动态连续性** | 讲解段无静止画面 | 每 3s 至少 1 个视觉变化 |
| **画面整洁度** | 统一配色、避免杂乱 | 同屏颜色 ≤ 4 种 |

### 0.2 观众体验目标

- **开场 3 秒法则**：配音 + 动画 + 音效同时出现，抓住注意力
- **信息密度适中**：避免"信息轰炸"，也避免"无意义留白"
- **视觉引导清晰**：观众知道该看哪里
- **节奏感明确**：重点慢、铺垫快、转场轻

### 0.3 技术质量标准

```
✅ 必须达成：
  - 代码可一次运行成功
  - 无 LaTeX 编译错误
  - 无布局越界/重叠
  - 时间轴无负时长/重叠

⭐ 推荐达成：
  - 可视化区存在 Updater 动态
  - 关键结论有高亮/Focus
  - Transform 衔接 ≥ 50%
  - 配音自然流畅
```

---

## 1. 工作流分层与质量门槛

### 1.1 三层工作流架构

```
┌─────────────────────────────────────────────────────────┐
│                    Layer 1: 输入层                       │
│    题目图片 / 视频解析 / PPT复刻 → 结构化信息提取          │
├─────────────────────────────────────────────────────────┤
│                    Layer 2: 编排层                       │
│    AI-1 生成 JSON 时间轴 → 人工校对 → 确定结构化脚本        │
├─────────────────────────────────────────────────────────┤
│                    Layer 3: 渲染层                       │
│    AI-2 生成 ManimGL 代码 → 调试优化 → 渲染输出            │
└─────────────────────────────────────────────────────────┘
```

### 1.2 输入层质量门槛

| 输入类型 | 预处理要求 | 质量检查点 |
|----------|------------|------------|
| **图片题** | 裁切/纠偏/去噪/高分辨率 | OCR 置信度 ≥ 80%，低置信度必须标注 |
| **视频解析** | 拆镜头/提取关键帧/转录字幕 | 时间戳精度 ≤ 0.5s |
| **PPT复刻** | 逐页导出/提取动画顺序 | 文本/公式完整无遗漏 |

### 1.3 JSON 时间轴质量门槛

```json
{
  "meta": {
    "title": "视频标题",
    "resolution": "1080x1920",
    "target_duration": [60, 120]
  },
  "timeline": [...]
}
```

**必须满足**：
- ✅ `start/end` 单调递增、无重叠
- ✅ 每句都有 `visual_action`，且可驱动画面
- ✅ 句长 < 26 字，超长必须拆句
- ✅ `narration` 口语化（TTS 友好），`screen_text/latex` 数学化
- ✅ 总时长符合 `target_duration` 范围

### 1.4 代码层质量门槛

```python
# 代码必须包含：
class MyScene(AutoScene):
    # ✅ 1. 常量配置区（位置/大小/颜色可调）
    TITLE_Y = 5.5
    VIZ_Y = 0.8
    
    # ✅ 2. 时间轴驱动器三件套
    def play_t(self, *anims, rt=1): ...
    def wait_t(self, dt): ...
    def wait_until(self, target): ...
    
    # ✅ 3. 阶段化组织
    def phase1_title(self): ...
    def phase2_problem(self): ...
    
    # ✅ 4. 动态 Updater（至少 1 个）
    # ✅ 5. Transform 衔接（相似物体）
    # ✅ 6. Indicate/高亮动作（关键结论）
```

---

## 2. 输入素材处理优化

### 2.1 图片题处理流程

```
原图 → OCR + 版面理解 → 可靠性标注 → 重排版题面 → Transform 过渡
```

**OCR 置信度处理策略**：

| 置信度 | 处理方式 |
|--------|----------|
| 高 (≥90%) | 直接使用 |
| 中 (70-90%) | 人工复核后使用 |
| 低 (<70%) | 必须标注 `【?】`，提供 A/B 两套方案 |

**题面重排版原则**：
- 标题（可选）：如"例题：xxx"
- 题干：按语义断行，避免一行过长
- 公式：全部给出可编译 LaTeX
- 小问：编号统一 (1)(2)(3)

### 2.2 视频/PPT 解析要点

**分镜脚本必须包含**：
- Shot 编号 + 时间范围
- 画面状态描述（哪些对象在画面上）
- 核心变化（出现/消失/移动/变形/高亮/相机运动）
- 讲解意图（这一镜头讲什么）

**对象复用关系**：
- 明确标注哪些对象跨镜头复用
- 复用对象用 Transform 衔接，而非 FadeOut + FadeIn

### 2.3 多题目处理

- 先拆分题号，每题独立处理
- 提供"总目录镜头"可选
- 题目切换时使用转场动画

---

## 3. 文案与时间轴优化

### 3.1 一句一动作原则

```
❌ 错误：一句话包含多个动作
   "首先设 x = a，代入方程，化简得到 x² = 4"

✅ 正确：拆分为多句
   "首先，我们设 x = a"          → visual_action: 写入设变量
   "代入原方程"                  → visual_action: 高亮方程
   "化简后得到 x² = 4"           → visual_action: Transform 推导
```

### 3.2 语速与停顿标准

| 段落类型 | 语速（字/秒） | 说明 |
|----------|--------------|------|
| intro | 5 | 开场节奏稍快，抓注意力 |
| middle | 4.5 | 正文讲解适中 |
| outro | 5 | 总结收尾干净利落 |

**停顿规则**：
- 句末停顿：0.2–0.4s（自然气口）
- 转场停顿：0.6–1.0s（单独事件，`visual_action="转场"`）
- TTS 气口：0.5s（AutoScene 默认 `VOICE_GAP_DURATION`）

### 3.3 配音与字幕分离

```python
# 配音口语化，字幕数学化
self.speak(
    text="由 f 2 等于 4",           # TTS 朗读（口语）
    subtitle="由 f(2) = 4",          # 屏幕显示（符号）
    color_map={"4": YELLOW}
)
```

**转换规则速查**：

| 类型 | 配音 (text) | 字幕 (subtitle) |
|------|-------------|-----------------|
| 负号 | `f 负 2 等于 负 4` | `f(-2) = -4` |
| 等号 | `a 等于 5` | `a = 5` |
| 坐标 | `坐标 2 4` | `(2, 4)` |
| 小于 | `x 小于 0` | `x < 0` |
| 平方 | `x 平方` | `x²` |
| 分数 | `三分之二` | `2/3` |

### 3.4 关键词上色策略

```python
# ✅ 正确：单一强调色，关键词少而精
self.speak("重点是奇函数性质", color_map={"奇函数性质": YELLOW})

# ❌ 错误：多色泛滥
self.speak("重点是奇函数性质", color_map={"重点": RED, "奇函数": BLUE, "性质": GREEN})
```

---

## 4. 画面排版与视觉层级

### 4.1 六块布局固定顺序

```
┌─────────────────────────────────┐  ← 顶部 y ≈ 5.5
│           Title (标题)           │
├─────────────────────────────────┤
│          Divider (分割线)        │
├─────────────────────────────────┤
│           Problem (题目)         │  ← y ≈ 3.5
├─────────────────────────────────┤
│         Viz (可视化区域)          │  ← y ≈ 0.8
├─────────────────────────────────┤
│        Derivation (推导区)       │  ← y ≈ -2.2
├─────────────────────────────────┤
│         Subtitle (字幕)          │  ← 底部 y ≈ -5.2
└─────────────────────────────────┘
```

### 4.2 字号层级配置

| 模块 | 建议字号 | 缩放参考 |
|------|----------|----------|
| Title | 40–48 | 1.0x |
| Problem | 28–34 | 0.75x |
| Derivation | 26–32 | 0.7x |
| Subtitle | 22–28 | 0.6x |
| 可视化标签 | 14–20 | 0.45x |

### 4.3 横向排版约束

```python
# 竖版视频参数
SCREEN_WIDTH = 27/4  # 6.75 Manim 单位
MARGIN_RATIO = 0.05  # 左右边距比例

# 安全宽度
m = min(0.2, MARGIN_RATIO * SCREEN_WIDTH)
W = SCREEN_WIDTH - 2 * m

# 目标填充比例
target_max = 0.95 * W   # 超过必须换行
target_fill = 0.92 * W  # 尽量填满
target_min = 0.85 * W   # 低于则放大
```

**避免"细长条"策略**：
1. 先换行控制 max_width
2. 换行后若宽度 < target_min，则 scale_to_fit_width(target_fill)

### 4.4 纵向间距自适应

```python
def layout_vertical(blocks, frame_h, buff_min=0.18, buff_max=0.45):
    """
    自适应垂直布局
    
    策略：
    1. 计算可用间距 gap_budget
    2. 均匀分配 buff
    3. 超出时按优先级缩放：Subtitle → Derivation → Problem
    4. Viz 保留最小高度 ≥ 0.28 * frame_h
    """
```

### 4.5 推导区与字幕防重叠

```python
# 使用 AutoScene 内置方法
self.ensure_above_subtitle(
    derivation_group,
    viz_bottom_y=viz.get_bottom()[1],
    margin=0.3,
    overlap_buff=0.2
)
```

**逻辑**：
- 重叠时：向上移动 + 额外 `overlap_buff`
- 无重叠时：居中放置于可视化底部和字幕顶部之间

---

## 5. 动画衔接与节奏控制

### 5.1 Transform 优先原则

```python
# ✅ 推荐：相似物体用 Transform
old_eq = Tex(r"x^2 + 2x")
new_eq = Tex(r"x^2 + 2x + 1")
self.play(TransformMatchingTex(old_eq, new_eq))

# ❌ 避免：无意义 FadeOut + FadeIn
self.play(FadeOut(old_eq))
self.play(FadeIn(new_eq))
```

**Transform 类型选择**：

| 场景 | 推荐动画 |
|------|----------|
| 公式微调 | `TransformMatchingTex` |
| 相似形状 | `TransformMatchingShapes` |
| 文本变化 | `TransformMatchingStrings` |
| 完全不同 | `ReplacementTransform` / `FadeTransform` |
| 首次出现 | `Write` / `FadeIn` |

### 5.2 渐进展示策略

```python
# 同屏信息密度控制：超过 6 个元素时分批显示
if len(new_elements) > 6:
    # 分批 FadeOut 旧内容，FadeIn 新内容
    self.play(FadeOut(batch_1))
    self.play(FadeIn(batch_2))
```

### 5.3 变速节奏设计

| 内容类型 | run_time | rate_func |
|----------|----------|-----------|
| 重点公式 | 2.0–3.0s | smooth |
| 铺垫过渡 | 0.5–1.0s | smooth |
| 转场清空 | 0.3–0.5s | smooth |
| 高亮闪烁 | 0.5–0.8s | there_and_back |

### 5.4 动画默认参数

```python
# 所有 self.play() 显式指定参数
self.play(Write(formula), rate_func=smooth, run_time=2)

# 禁止隐式依赖默认值
```

---

## 6. 可视化区动态策略

### 6.1 强制 Updater 要求

**每个可视化区域至少包含 1 个持续更新的对象**，全程不能是"画面静止、只有字幕在变"。

### 6.2 动态展示模式（任选其一）

```python
# 模式 1：ValueTracker + DecimalNumber
b_tracker = ValueTracker(0)
decimal = DecimalNumber(0, num_decimal_places=2)
decimal.add_updater(lambda d: d.set_value(b_tracker.get_value()))
self.play(b_tracker.animate.set_value(4), run_time=3)

# 模式 2：曲线/点随参数变化
def get_dynamic_curve():
    b = b_tracker.get_value()
    return axes.get_graph(lambda x: x**2 + b*x, x_range=[-5, 5])
curve = always_redraw(get_dynamic_curve)

# 模式 3：游标沿路径移动
cursor = Dot(color=YELLOW)
cursor.add_updater(lambda c: c.move_to(path.point_from_proportion(t.get_value())))

# 模式 4：呼吸高亮（周期性 alpha/scale）
rect.add_updater(lambda r: r.set_stroke(width=2 + 2*np.sin(2*np.pi*t.get_value())))
```

### 6.3 Updater 与时间轴同步

```python
def add_dynamic_indicator(self, viz_group, t_start, t_end):
    """每个时间段重置 tracker 目标值，与讲解同步"""
    t = ValueTracker(0)
    
    # 绑定 updater
    decimal.add_updater(lambda d: d.set_value(10 * t.get_value()))
    
    # 与时间轴对齐
    self.play(
        t.animate.set_value(1),
        run_time=t_end - t_start,
        rate_func=linear
    )
```

### 6.4 静态内容必须补轻量动态

即使本段内容本质静态，也必须加入"轻量动态"：
- 关键框呼吸高亮（周期性 stroke_width/opacity）
- 时间轴游标移动
- 扫描线从左到右
- 相机微平移（建议用 updater 而非 animate）

---

## 7. 字幕与配音系统

### 7.1 AutoScene 字幕管线

```python
# 方式 1：speak() 自动配音 + 字幕
duration = self.speak("欢迎观看本视频")

# 方式 2：speak() 配音 + 高亮（使用 targets 参数）
# 注：原 speak_with_highlight 已弃用
self.speak(
    text="看这个公式",
    subtitle="看这个公式",
    targets=[formula],  # 高亮对象列表
    color_map={"公式": YELLOW}
)

# 方式 3：speak_sequence() 批量配音
self.speak_sequence([
    "第一句话",
    "第二句话",
    {"text": "第三句", "color_map": {"第三句": RED}},
])

# 方式 4：run_timeline() 完整时间轴
self.run_timeline(events, generate_voice=True)
```

### 7.2 字幕样式配置

```python
self.set_subtitle_style(
    font_size=22,
    edge_buff=0.3,
    max_chars=18  # 竖版每行字符数
)
```

### 7.3 字幕动画规则

```
第 1 句：Write(subtitle)
第 2+ 句：Transform(old_subtitle, new_subtitle)
最后清除：FadeOut(subtitle)
```

### 7.4 字幕与上屏公式配合

```python
# 先字幕后公式（推荐）
self.speak("接下来看公式")
self.play(Write(formula))

# 或并行出现（使用 targets 参数）
self.speak("看这个公式", targets=[formula])
```

---

## 8. 相机与构图控制

### 8.1 相机使用原则

- ✅ 只在"强调局部"时使用相机移动
- ✅ 完整节奏：开始 → 停留 → 恢复
- ❌ 避免频繁抖动
- ❌ 避免无目的漂移

### 8.2 camera_focus 使用

```python
# 聚焦后恢复
self.camera_focus(
    target=dot,
    zoom_factor=3.0,
    focus_time=0.8,
    hold_time=1.5,
    restore_time=0.8
)
```

### 8.3 focus_guide_with_camera 使用

```python
# 方框引导 + 相机跟随
self.focus_guide_with_camera(
    targets=[formula["a^2"], formula["b^2"], formula["c^2"]],
    zoom_factor=2.5,
    hold_time=1.5,
    restore_after=True
)
```

### 8.4 固定元素 fix_in_frame

```python
# 必须固定的元素（相机移动时不变）
title.fix_in_frame()
subtitle.fix_in_frame()
divider.fix_in_frame()

# 不固定的元素（跟随相机）
# 可视化区域、部分推导内容
```

---

## 9. 音效与配乐策略

### 9.1 SoundLibrary 使用

```python
# 启用动画音效
self.set_animation_sounds_enabled(True)
self.set_add_sounds_enabled(False)  # add() 音效通常关闭
self.set_sound_gain(0.6)  # 音量 60%
```

### 9.2 音效触发原则

| 动画类型 | 音效文件夹 | 使用场景 |
|----------|------------|----------|
| Write/ShowCreation | creation/ | 首次写入 |
| FadeIn/FadeOut | fade/ | 出现/消失 |
| Transform | transform/ | 形态变化 |
| Indicate/Flash | indicate/ | 强调高亮 |
| GrowArrow | grow/ | 箭头生长 |

### 9.3 音效使用规范

- ✅ 只在关键动作触发，不要全程铺满
- ✅ 音效增益统一控制（0.4–0.6）
- ✅ 配音优先，音效辅助
- ❌ 避免音效与配音冲突
- ❌ 避免连续重复音效

### 9.4 背景音乐（如有）

- BGM 音量需低频弱化
- 配音段 BGM 自动降低
- 重点段落可暂停 BGM

---

## 10. 视觉引导与标注系统

### 10.1 focus_guide 方框引导

```python
# 依次高亮多个目标
self.focus_guide(
    targets=[formula["a"], formula["b"], formula["c"]],
    hold_time=1.0,
    box_buff=0.1,
    stroke_width=3
)

# 或按关键词引导
self.focus_guide_sequence(
    text_mobject=sentence,
    keywords=["向量", "加法", "交换律"],
    hold_time=1.0
)
```

### 10.2 弯曲箭头标注

```python
# 单个标注
ann = self.add_curved_annotation(
    target=formula["c^2"],
    annotation="斜边的平方",
    direction="ur"  # 右上方向
)

# 批量标注
annotations = self.add_multi_curved_annotations([
    {"target": formula["a^2"], "text": "直角边a", "direction": "ul"},
    {"target": formula["b^2"], "text": "直角边b", "direction": "up"},
    {"target": formula["c^2"], "text": "斜边", "direction": "ur"},
])
```

### 10.3 区域标注 annotate_region

```python
# 纯色背景覆盖 + 标注
ann = self.annotate_region(
    region=circle,
    annotation="这是一个圆",
    bg_color=BLUE
)
```

### 10.4 highlight_text 高亮效果

```python
# 可选效果：box/underline/background/indicate/focus/wave/random
decoration = self.highlight_text(formula, effect="box")

# 移除高亮
self.remove_highlight(decoration)
```

### 10.5 固定标注（3D 场景）

```python
# 始终面向相机的标注
label = self.add_fixed_annotation(sphere, "球体", direction=DOWN)

# 动态跟随的标注
dynamic_label = self.add_fixed_annotation_dynamic(moving_obj, "跟随")
```

---

## 11. 辉光与特效使用

### 11.1 辉光文字/公式

```python
# 辉光文字
glow_text = self.create_glow_text("标题", color=YELLOW)

# 辉光公式
glow_tex = self.create_glow_tex(r"E=mc^2", color=BLUE)
```

### 11.2 曲线辉光

```python
# 给曲线添加辉光
glow = self.add_glow_to_curve(circle, glow_factor=2.0)

# 脉动辉光曲线
pulse_curve = self.create_pulse_glow_curve(
    start=LEFT, end=RIGHT, color=YELLOW
)
```

### 11.3 辉光使用原则

- ✅ 用于标题、关键结论、重点公式
- ✅ 辉光颜色与文字颜色协调
- ❌ 避免全屏辉光
- ❌ 避免辉光叠加（性能问题）

---

## 12. 颜色与配色策略

### 12.1 默认配色方案

```python
# 基础色
TEXT_COLOR = WHITE       # 默认文字
ACCENT_COLOR = YELLOW    # 强调色（唯一）
HIGHLIGHT_COLOR = RED    # 高亮色（短暂使用）

# 可视化色
FUNC_COLOR_NEG = BLUE    # x < 0 部分
FUNC_COLOR_POS = RED     # x > 0 部分
AXIS_COLOR = WHITE       # 坐标轴
```

### 12.2 强调色使用规则

```python
# 强调约 2s 后恢复
self.play(formula.animate.set_color(RED), run_time=0.5)
self.wait(2)
self.play(formula.animate.set_color(WHITE), run_time=0.5)
```

### 12.3 禁止的配色行为

- ❌ 同屏超过 4 种颜色
- ❌ 连续闪烁变色
- ❌ 大面积彩色背景
- ❌ 红绿搭配（色盲不友好）

### 12.4 Text 上色方式

```python
# 使用 t2c / text2color
text = Text(
    "重点是向量",
    font="STKaiti",
    t2c={"重点": RED, "向量": YELLOW}
)

# 或使用 color_map（字幕）
self.speak("重点是向量", color_map={"重点": RED})
```

---

## 13. 代码组织与可维护性

### 13.1 类结构模板

```python
class MyScene(AutoScene):
    # ========== 常量配置区 ==========
    # 每个物体都要有可调试的位置/大小入口
    TITLE_Y = 5.5          # 标题 Y 坐标
    PROBLEM_Y = 3.5        # 题目 Y 坐标
    VIZ_Y = 0.8            # 可视化 Y 坐标
    SOLUTION_Y = -2.2      # 解答 Y 坐标
    
    SCREEN_WIDTH = 27/4    # 竖版安全宽度
    
    TITLE_FONT_SIZE = 24
    PROBLEM_FONT_SIZE = 24
    SOLUTION_FONT_SIZE = 24
    
    # ========== 颜色配置 ==========
    HIGHLIGHT_COLOR = YELLOW
    ACCENT_COLOR = RED
    
    def construct(self):
        """主流程调度（只做流程控制）"""
        self.setup_scene()
        self.phase1_title()
        self.phase2_problem()
        self.phase3_viz()
        self.phase4_derivation()
        self.phase5_ending()
    
    def setup_scene(self):
        """场景初始化"""
        self.enable_debug(True)
        self.set_animation_sounds_enabled(True)
        self.set_subtitle_style(font_size=22, edge_buff=0.3)
        self.shared_objects = {}  # 跨阶段共享对象
    
    def phase1_title(self):
        """阶段 1：标题展示"""
        # 前半部分：创建对象并摆放
        title = Text("标题", font="STKaiti", font_size=self.TITLE_FONT_SIZE)
        title.move_to(UP * self.TITLE_Y)
        
        # 后半部分：统一动画
        self.play(Write(title), rate_func=smooth, run_time=1)
        self.speak("欢迎观看")
        
        # 保存跨阶段对象
        self.shared_objects["title"] = title
```

### 13.2 阶段独立可调试

```python
def construct(self):
    self.setup_scene()
    # 注释其他阶段可单独调试
    self.phase1_title()
    # self.phase2_problem()
    # self.phase3_viz()
```

### 13.3 对象清理规则

```python
def phase2_problem(self):
    # 阶段开始：检查前置条件
    if "title" not in self.shared_objects:
        raise ValueError("需要先执行 phase1_title")
    
    # ... 阶段逻辑 ...
    
    # 阶段结束：清理临时对象，保留共享对象
    self.remove(temp_annotation)
    self.shared_objects["problem"] = problem_group
```

### 13.4 遮挡关系控制

```python
# 后 add 的对象在上层
self.add(background)      # 最底层
self.add(axes)            # 中间层
self.add(curve)           # 上层
self.add(annotation)      # 最上层
```

---

## 14. 性能与渲染稳定性

### 14.1 always_redraw 性能控制

```python
# ✅ 必要时使用 always_redraw
curve = always_redraw(get_dynamic_curve)

# ⚠️ 数量受控（建议 ≤ 5 个）
# ⚠️ 动画结束后转为静态
if animation_done:
    static_curve = get_dynamic_curve()
    self.remove(curve)
    self.add(static_curve)
```

### 14.2 复杂度控制

| 元素 | 建议限制 |
|------|----------|
| always_redraw 对象 | ≤ 5 个 |
| 同屏点数 | ≤ 1000 |
| 辉光层数 | ≤ 3 层 |
| 描边复杂度 | stroke_width ≤ 6 |

### 14.3 渲染前预览流程

```bash
# 1. 小分辨率预览（确认时序）
manimgl script.py SceneName --resolution 480x854

# 2. 确认无误后高分辨率渲染
manimgl script.py SceneName -w --resolution 1080x1920
```

### 14.4 自检输出

```python
def construct(self):
    # 渲染前自检
    print("="*50)
    print(f"视频标题: {self.meta['title']}")
    print(f"目标时长: {self.timeline[-1]['end']}s")
    print("="*50)
    
    for e in self.timeline:
        duration = e['end'] - e['start']
        if duration < 0:
            raise ValueError(f"负时长！事件: {e['scene_id']}")
        print(f"[{e['start']:.1f}-{e['end']:.1f}] {e['visual_action']}: {e['narration'][:20]}...")
```

---

## 15. 自动化工具与检查器

### 15.1 JSON Schema 校验器

```python
def validate_timeline_json(data: dict) -> list[str]:
    """
    校验时间轴 JSON
    
    检查项：
    - meta 字段完整性
    - start/end 单调递增
    - 无负时长
    - visual_action 覆盖率 100%
    - 句长 ≤ 26 字
    """
    errors = []
    
    # ... 校验逻辑 ...
    
    return errors
```

### 15.2 布局检查器

```python
def check_layout(scene) -> list[str]:
    """
    检查布局问题
    
    检查项：
    - 元素是否越界
    - 元素是否重叠
    - 行宽是否过窄（< 85%）
    - 推导区是否压住字幕
    """
```

### 15.3 动态检查器

```python
def check_dynamics(scene) -> list[str]:
    """
    检查动态元素
    
    检查项：
    - 是否存在 Updater
    - 是否存在 always_redraw
    - 是否存在 ValueTracker
    """
```

### 15.4 时间轴可视化预览

```python
def visualize_timeline(events: list) -> None:
    """
    生成时间轴条形图
    
    显示：
    - 每句 narration
    - 对应 visual_action
    - 时间段长度
    """
```

---

## 16. 交付前检查清单

### 16.1 必检项 ✅

- [ ] 时间轴无重叠、无负时长
- [ ] 代码可一次运行成功
- [ ] 无 LaTeX 编译错误
- [ ] 字幕与配音同步（误差 ≤ 0.2s）
- [ ] 画面无超宽/越界
- [ ] 推导区不压住字幕

### 16.2 推荐项 ⭐

- [ ] 可视化区存在持续动态（Updater）
- [ ] 关键结论有高亮/框选/相机聚焦
- [ ] 颜色体系一致（WHITE + 单一强调色）
- [ ] Transform 衔接 ≥ 50%
- [ ] 开场 3 秒：配音 + 动画 + 音效同时出现
- [ ] 配音自然流畅（无卡顿/无杂音）

### 16.3 加分项 🌟

- [ ] 可视化标签使用彩色背景突出
- [ ] 辉光效果用于关键元素
- [ ] 相机聚焦用于强调局部
- [ ] 音效与动画类型匹配
- [ ] 代码注释清晰完整

---

## 17. 常见问题与解决方案

### 17.1 LaTeX 编译错误

```python
# 问题：\div 在 ManimGL 中不支持
# 解决：使用 ÷ 替代
div = Tex(r"6 \div 2 = 3")  # ❌
div = Tex("6÷2=3")          # ✅
```

### 17.2 中文显示问题

```python
# 问题：中文乱码或不显示
# 解决：指定字体
text = Text("中文内容", font="STKaiti")  # ✅
```

### 17.3 布局越界

```python
# 问题：文本超出屏幕
# 解决：使用 auto_wrap 或 scale_to_fit_width
if mobject.get_width() > SCREEN_WIDTH:
    mobject.scale_to_fit_width(SCREEN_WIDTH * 0.95)
```

### 17.4 时间轴不同步

```python
# 问题：音画不同步
# 解决：使用时间轴驱动器三件套
def play_t(self, *anims, rt=1):
    self.play(*anims, run_time=rt, rate_func=smooth)
    self.t += rt

def wait_t(self, dt):
    self.wait(dt)
    self.t += dt

def wait_until(self, target):
    if target > self.t:
        self.wait_t(target - self.t)
```

### 17.5 always_redraw 性能问题

```python
# 问题：帧率下降
# 解决：动画结束后转为静态
# 或减少 always_redraw 数量
```

### 17.6 音效不播放

```python
# 问题：SoundLibrary 未找到
# 解决：检查路径配置
self._sound_library = SoundLibrary(
    library_path="E:/auto_manim/assets/sounds/library"
)
```

---

## 18. 后续可落地优化项

### 18.1 短期优化（1-2 周）

- [ ] **auto_wrap Tex 安全断行**
  - 不拆坏 `\command{...}` 结构
  - 不拆坏大括号配对
  - 支持混合中英文

- [ ] **统一风格配置文件**
  ```yaml
  # style_config.yaml
  fonts:
    chinese: STKaiti
    english: Arial
  sizes:
    title: 48
    body: 36
    subtitle: 28
  colors:
    text: "#FFFFFF"
    accent: "#FFFF00"
    highlight: "#FF0000"
  ```

### 18.2 中期优化（1-2 月）

- [ ] **模板场景库沉淀**
  - 函数图像题模板
  - 几何证明题模板
  - 概率统计题模板
  - 不等式推导模板

- [ ] **JSON Schema 校验工具**
  - 命令行工具
  - 自动修复建议

### 18.3 长期优化（3-6 月）

- [ ] **时间轴可视化编辑器**
  - 拖拽调整时间
  - 预览字幕
  - 导出 JSON

- [ ] **自动布局引擎**
  - 智能避让
  - 自动缩放
  - 自动对齐

- [ ] **AI 代码质量检查**
  - 自动检测常见错误
  - 优化建议
  - 风格一致性检查

---

## 📝 附录

### A. 渲染命令速查

```bash
# 预览模式
manimgl script.py SceneName

# 输出视频
manimgl script.py SceneName -w

# 竖版视频 (9:16)
manimgl script.py SceneName -w --resolution 1080x1920

# 横版视频 (16:9)
manimgl script.py SceneName -w --resolution 1920x1080

# 低分辨率预览
manimgl script.py SceneName --resolution 480x854
```

### B. AutoScene API 速查

| 方法 | 用途 |
|------|------|
| `speak(text)` | 配音 + 字幕 |
| `speak_with_highlight(text, targets)` | 配音 + 高亮（⚠️已弃用，使用 `speak(text, targets)` 代替） |
| `focus_guide(targets)` | 方框引导 |
| `focus_guide_with_camera(targets)` | 方框 + 相机 |
| `camera_focus(target)` | 相机聚焦 |
| `add_curved_annotation(target, text)` | 弯曲箭头标注 |
| `highlight_text(target, effect)` | 高亮效果 |
| `ensure_above_subtitle(mobject)` | 防字幕重叠 |

### C. 颜色常量速查

```python
from manimlib import (
    WHITE, BLACK, GREY,
    RED, BLUE, GREEN, YELLOW,
    ORANGE, PINK, PURPLE, TEAL, GOLD,
    UP, DOWN, LEFT, RIGHT, ORIGIN,
    UL, UR, DL, DR
)
```

---

*文档版本：v2.0*
*更新日期：2025-12-22*
*维护者：auto_manim 项目组*
