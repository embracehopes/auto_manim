...
// speed of ROTATION - 移除重复定义，使用下面配置区域的定义
...
// 相机焦距/视野 - 数值越小视野越广，越大视野越窄
#define FDIST 0.7                    // 👈 调整视野大小

// 立方体尺寸 - 调整为1:1:1比例的立方体
#define BOXDIMS vec3(1.0, 1.0, 1.0)  // 👈 1:1:1比例的正立方体

// 光学参数
#define IOR 1.33                     // 👈 折射率 - 影响玻璃折射效果

// 相机位置控制参数 - 侧面专用：设为侧视角度
#define CAMERA_POS 0.5           // 👈 侧视角度，0.5表示90度侧面视角  
//#define CAMERA_FAR 0.1             // 👈 相机距离 (默认2.0, 可以0.5或10.0等)

// 旋转速度控制 - 设为0停止旋转，实现静态正视角度
#define ROTATION_SPEED 0.0        // 👈 旋转速度设为0，静态显示

// 形状变化参数 (取消注释来启用)
//#define STATIC_SHAPE 0.15          // 👈 静态形状参数 (默认0.5)
//#define ANIM_SHAPE                 // 👈 启用形状动画
//#define ANIM_COLOR                 // 👈 启用颜色动画

// 渲染质量设置
//#define AA_CUBE 4                  // 👈 抗锯齿级别 (2-4, 会降低性能)
...
// Uniform replacements for Shadertoy variables
// 👈 注意：iResolution会影响长宽比，这里使用正方形比例避免拉伸
vec3 iResolution = vec3(1080.0, 1080.0, 1.0);  // 👈 使用正方形分辨率避免拉伸
float iTime = time;
...
vec4 insides(vec3 ro, vec3 rd, vec3 nor_c, vec3 l_dir, out float tout)
{
    ...
    float a = 1.;
    if (dx[0].x < dx[1].x){{vec3 tv;swap(dx[0], dx[1]);}{int tv;swap(order[0], order[1]);}}
    if (dx[1].x < dx[2].x){{vec3 tv;swap(dx[1], dx[2]);}{int tv;swap(order[1], order[2]);}}
    if (dx[0].x < dx[1].x){{vec3 tv;swap(dx[0], dx[1]);}{int tv;swap(order[0], order[1]);}}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // ================================
    float osc = 0.5;
    vec3 l_dir = normalize(vec3(0., 1., 0.));   // 👈 光源方向
    l_dir *= rotz(0.5);                         // 👈 光源旋转角度
...
#endif
#ifdef CAMERA_POS
    mouseY = PI*CAMERA_POS;                     // 👈 使用静态相机位置
#else
    // 👈 动态相机垂直运动 - 可以调整这些参数改变相机上下运动
    ...
    float mouseX = -2.*PI-0.25*(iTime*ROTATION_SPEED+tshift);  // 👈 使用自定义旋转速度
#else
    float mouseX = -2.*PI-0.25*(iTime+tshift);                 // 👈 默认旋转速度
#endif
#ifdef MOUSE_control
    ...
    // ================================
#ifdef CAMERA_FAR
    vec3 eye = (2. + CAMERA_FAR) * vec3(cos(mouseX) * cos(mouseY), sin(mouseX) * cos(mouseY), sin(mouseY));  // 👈 使用自定义距离
#else
    vec3 eye = 4. * vec3(cos(mouseX) * cos(mouseY), sin(mouseX) * cos(mouseY), sin(mouseY));  // 👈 默认距离=4.0，可以直接修改这个数字
#endif
...
#if defined(BG_ALPHA)||defined(ONLY_BOX)||defined(SHADOW_ALPHA)
    // ManimGL doesn't have iChannel0, so we'll use a simple black background
    fragColor.rgb=fragColor.rgb*fragColor.w+vec3(0.0)*(1.-fragColor.w);
#endif
...
void main() {
    ...
    // 🎯 坐标转换 - 修复长宽比拉伸问题
    // ================================
    // v_uv 来自 ManimGL，范围是 [0,1]
    // 转换为 fragCoord 用于 mainImage
    vec2 fragCoord = v_uv * iResolution.xy;
    
    // 为了确保正确的长宽比，我们可以在这里做额外的调整
    // 如果还有拉伸问题，可以尝试取消下面这行的注释：
    // fragCoord.x *= (iResolution.y / iResolution.x);  // 👈 强制正方形比例
