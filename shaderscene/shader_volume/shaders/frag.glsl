...
mat3 cameraRotation() {
    return mouseRotation(MOUSE_CONTROL==2, CAMERA_ROTATION);
}
...
float animSubdivisions2() {
    return mix(1., 2.3, sin(time_local * PI/2.) * .5 + .5);
}

HexSpec animHex2(vec3 hexCenter, float subdivisions) {
    HexSpec spec = newHexSpec(subdivisions);
    float blend = hexCenter.y;
    spec.height = mix(1.6, 2., sin(blend * 10. + time_local * PI) * .5 + .5);
...
HexSpec animHex3(vec3 hexCenter, float subdivisions) {
    ...
    blend = cos(blend + time_local * PI) * .5 + .5;
...
float transitionValues(float a, float b, float c) {
    float t = time_local / SCENE_DURATION;
...
Model geodesicModel(vec3 p) {
    ...
    model = hexModel(p, points.b, edgeAB, edgeBC, spec);
    spec = transitionHexSpecs(
        animHex1(points.c, subdivisions),
        animHex2(points.c, subdivisions),
        animHex3(points.c, subdivisions)
    );
    Model part = hexModel(p, points.c, edgeBC, edgeCA, spec);
...
Model mapScene( vec3 p ){
    mat3 m = modelRotation();
    p *= m;  
    pR(p.xz, time_local * PI/16.);
...
vec3 calcNormal( in vec3 pos ){
    ...
    vec3 nor = vec3(
        mapScene(pos+eps.xyy).dist - mapScene(pos-eps.xyy).dist,
        mapScene(pos+eps.yxy).dist - mapScene(pos-eps.yxy).dist,
        mapScene(pos+eps.yyx).dist - mapScene(pos-eps.yyx).dist );
...
Hit raymarch(CastRay castRay){
    ...
    for( int i=0; i< NUM_OF_TRACE_STEPS ; i++ ){
        ...
        model = mapScene(ray.origin + ray.direction * ray.len);
...
void doCamera(out vec3 camPos, out vec3 camTar, out float camRoll, in float t, in vec2 mouseNorm) {
    float dist = 5.5;
    camRoll = 0.;
    camTar = vec3(0,0,0);
...
vec3 linearToScreen(vec3 linearRGB) {
    return gammaCorrect(linearRGB, 1.0 / GAMMA);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float camRoll = 0.;
    doCamera(camPos, camTar, camRoll, time_local, m);
