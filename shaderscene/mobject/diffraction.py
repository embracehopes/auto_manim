...
该模块实现了基于 GPU Shader 的光波干涉与衍射效果可视化。
主要功能包括：
1. 光波衍射与干涉的实时计算和渲染
2. 多点光源的波叠加效应
3. HSL 到 RGB 颜色空间转换
4. 基于 moderngl 的自定义 shader 渲染
...
def hsl_to_rgb(hsl):
    """
    将 HSL 颜色值数组转换为 RGB 颜色值数组
    
    HSL 颜色模型便于创建平滑的颜色变换效果，特别适用于:
    - 色相（Hue）的循环变化
    - 饱和度（Saturation）的渐变
    - 亮度（Lightness）的动态调整

    Args:
        hsl (np.ndarray): 形状为 (n, 3) 的 numpy 数组，每行代表一个 HSL 值
                         (色相 [0, 1), 饱和度 [0, 1], 亮度 [0, 1])

    Returns:
        np.ndarray: 形状为 (n, 3) 的数组，包含范围在 [0, 1] 的 RGB 值
        
    数学原理：
        HSL 到 RGB 的转换基于色相环的几何关系，
        通过分段线性插值在 RGB 立方体中计算对应点
    """
    h = hsl[:, 0]  # 色相分量：决定颜色类型（红、绿、蓝等）
    s = hsl[:, 1]  # 饱和度分量：决定颜色的纯度
    l = hsl[:, 2]  # 亮度分量：决定颜色的明暗程度
    ...
    def hue_to_rgb(p, q, t):
        ...
        Returns:
            对应的 RGB 分量值
        """
        # 确保色相值在 [0, 1] 范围内（色相是循环的）
        t = np.where(t < 0, t + 1, np.where(t > 1, t - 1, t))
        # 分段线性插值，将色相映射到 RGB 空间
        return np.where(t < 1/6, p + (q - p) * 6 * t,      # 红到黄的过渡
               np.where(t < 1/2, q,                         # 黄到青的纯色段
               np.where(t < 2/3, p + (q - p) * (2/3 - t) * 6, p  # 青到品红的过渡
        )))
...
class LightWaveSlice(Mobject):
    ...
    该类使用 GPU Shader 实现多点光源的波干涉效应可视化。
    主要特性：
    1. 基于 GLSL Fragment Shader 的实时波计算
    2. 支持多达 32 个点光源的同时渲染
    3. 可调节波长、频率、衰减等物理参数
    4. 支持波的振幅显示和强度显示两种模式
    ...
    - 使用惠更斯原理：每个点光源产生球面波
    - 波的叠加：多个波源的复数振幅相加
    - 衰减模型：距离的幂函数衰减 (1+r)^(-decay_factor)
    
    渲染技术：
    - 使用 moderngl.TRIANGLE_STRIP 几何体
    - Fragment Shader 中并行计算每个像素的波值
    - 实时更新时间参数实现动画效果
    """
    # Shader 文件路径：指向包含 GLSL 着色器的目录
    shader_folder: str = str(Path(Path(__file__).parent.parent, "diffraction_shader"))
    
    # 顶点数据结构：定义传递给 GPU 的数据格式
    data_dtype: Sequence[Tuple[str, type, Tuple[int]]] = [
        ('point', np.float32, (3,)),  # 3D 坐标点 (x, y, z)
    ]
    
    # 渲染图元类型：使用三角形条带进行高效渲染
